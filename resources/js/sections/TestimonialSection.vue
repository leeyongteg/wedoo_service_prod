<template>
    <section ref="testimonialsection">
        <Swiper class="swiper-container" v-if="testimonial_data.length > 0" :modules="modules" :slides-per-view="3"
            :space-between="30" :pagination="{ clickable: true }" :loop="true"
            ...
            <SwiperSlide v-for="data in testimonial_data" :key="data">
                <TestimonialCard :is-light-bg="data.isLightBg" :is-rating="data.isRating" :rating="data.rating"
                    :title="data.service_name" :content="data.review" :quote-image="data.quoteImage"
                    :user-image="data.profile_image" :user-name="data.customer_name" :user-meta="data.created_at" />
    ...
    <SwiperSlide  v-for="item in 8" :key="item">
        ...
        <div v-if="testimonial_data.length == 0 && loading == 0"
            class="row row-cols-xl-4 row-cols-md-3 row-cols-sm-2 row-cols-1 justify-content-center mt-5 ">
