<template>
  <section ref="servicesection">
        ...
        <div class="mt-5 justify-content-center service-slide-items-4">
          <div class="col">
            <ServiceCard
              ...
              :min_price_range="service.min_price_range"
              :max_price_range="service.max_price_range"
    ...
    <Swiper
      class="swiper-container"
      v-if="IS_LOADER == true && serviceDetails.length == 0"
      :modules="modules"
      :slides-per-view="4"
      :space-between="30"
      :pagination="{ clickable: true }"
...
const [servicesection] = useObserveSection(() =>
  store.get_service_list({ per_page: 8, category_id: null })
)
...
const fetchProviders = async () => {
  try {
    const response = await fetch(
      PROVIDER_API({ user_type: 'provider', status: 1, per_page: 'all' })
    )
    const data = await response.json()
  ...
  try {
    ...
    const settings = store.landing_page_setting_list_data.data.find(
      (setting) =>
        ...
        (props.type === 'recently_view' && setting.key === 'section_8' && setting.status === 1)
    )
    ...
    if (settings) {
      ...
      const selectedServices = allService.filter((service) =>
        serviceIds.includes(String(service.id))
      )
...
const isFavourite = (serviceId) => {
  return props.favourite !== null
    ? props.favourite.some((item) => item.service_id === serviceId)
    : false
}
