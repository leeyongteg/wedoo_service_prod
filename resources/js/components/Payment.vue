      ...
      <form @submit="formSubmit">
        <template v-if="!isChildComponentVisible">
          ...
          <div class="d-flex align-items-center flex-wrap gap-3">
            <div v-for="gateway in paymentGatewaysToDisplay" :key="gateway.id" class="form-check">
              <input
                class="form-check-input"
                type="radio"
                :name="'payment_method_' + gateway.type"
                v-model="payment_method"
                :id="gateway.type"
                :value="gateway.type"
                :checked="payment_method === gateway.type"
              />
              <label class="form-check-label h6 fw-normal text-capitalize" :for="gateway.type">{{
                $t('messages.' + gateway.type)
              }}</label>
            </div>
            ...
            <div class="form-check">
              ...
              <label class="form-check-label h6 fw-normal text-capitalize" for="wallet">{{
                $t('messages.wallet')
              }}</label>
            </div>
          </div>
...
import { confirmcancleSwal, confirmcancleWallet, formatCurrencyVue } from '../data/utilities'
...
const hideModal = () => {
  if (isChildComponentVisible.value) {
...
const isStripeEnabled = computed(() => {
  return (
    Array.isArray(paymentGatewayList.value) &&
    paymentGatewayList.value.some((gateway) => gateway.type === 'stripe' && gateway.status === 1)
  )
})
  ...
  if (values.payment_type == 'wallet') {
    if (values.wallet_amount > 0 && values.total_amount < values.wallet_amount) {
      ...
      confirmcancleWallet({ title: title }).then(async (result) => {
        ...
        if (!result.isConfirmed) return
        IsLoading.value = 1
        showChildComponent(values.payment_type)
      })
    }
  } else {
    IsLoading.value = 1
    ...
    if (response.ok) {
      ...
      if (responseData.payment_geteway_data != null && responseData.payment_type !== null) {
        if (responseData.payment_type == 'stripe') {
          Openstripepayment(responseData)
        } else if (responseData.payment_type == 'freemopay') {
...
const showChildComponent = (payement_type) => {
  if (payement_type == 'wallet') {
    currentComponent.value = Wallet
  } else if (payement_type == 'freemopay') {
    currentComponent.value = PhoneNumberModal
  }
  isChildComponentVisible.value = true
}

// Définir les types de paiement autorisés
...
const paymentGatewaysToDisplay = computed(() => {
  if (!Array.isArray(paymentGatewayList.value)) return []

  return paymentGatewayList.value.filter((gateway) => {
    // Vérifier si la passerelle a un statut actif (status === 1) et si son type est autorisé
    return gateway.status === 1 && allowedPaymentTypes.includes(gateway.type)
  })
})
</script>
