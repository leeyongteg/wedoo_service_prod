<template>
    <div class="service-box-card bg-light rounded-3 mb-5">
        ...
        <a :href="`${baseUrl}/service-detail/${service_id}`" class="service-heading mt-4 d-block p-0"
            @click="storeRecentlyViewed">
        ...
        <ul class="list-inline p-0 mt-1 mb-0 price-content">
            <li class="text-primary fw-500 d-inline-block position-relative font-size-18">
                <span v-if="price == 0 && type_service == 'free'"
                    class="text-primary fw-500 d-inline-block position-relative font-size-18">Free</span>
                <span v-else-if="type_service === 'fixed'"
                    class="text-primary fw-500 d-inline-block position-relative font-size-18">
                    {{ formatNumberWithSpace(min_price_range) + '.00' }} - {{ formatNumberWithSpace(max_price_range) +
                    '.00 ' }} {{ currencySymbol }} :
                    {{ capitalizeFirstLetter(type_service) }}
                </span>
                <span v-else-if="price !== 0 && type_service == 'hourly'"
                    class="text-primary fw-500 d-inline-block position-relative font-size-18">
        ...
        <div class="mt-3">
            <div class="d-flex align-items-center gap-1 f-none mt-2">
                ...
                <h6 class="font-size-14">
                    {{ reviewNo }}
                    <a :href="`${baseUrl}/rating-all?service_id=${service_id}`"><span v-if="reviewCount > 1"
                            class="text-body ms-1">({{ reviewCount }} {{ $t('messages.reviews') }})</span><span v-else
                            class="text-body ms-1">({{ reviewCount }} {{ $t('messages.review') }})</span>
                    </a>
                </h6>
            </div>
        </div>
...
const props = defineProps({
    ...
    max_price_range: { type: Number, default: 0 },
    min_price_range: { type: Number, default: 0 },
    type_service: { type: String, default: '' }
})
