<template>
  <div class="service-box-card bg-light rounded-3 mb-5">
    <div class="iq-image position-relative">
      ...
      <a
        :href="`${baseUrl}/service-detail/${service_id}`"
        @click="storeRecentlyViewed"
        class="service-img"
      >
        <img
          :src="image ? image : baseUrl + '/images/default.png'"
          alt="service"
    ...
    <ul class="list-inline p-0 mt-1 mb-0 price-content">
      <li class="text-primary fw-500 d-inline-block position-relative font-size-18">
        <span
          v-if="price == 0 && type_service == 'free'"
        ...
        <h6 class="font-size-14">
          ...
          <a :href="`${baseUrl}/rating-all?service_id=${service_id}`"
            ><span v-if="reviewCount > 1" class="text-body ms-1"
              >({{ reviewCount }} {{ $t('messages.reviews') }})</span
            ><span v-else class="text-body ms-1"
              >({{ reviewCount }} {{ $t('messages.review') }})</span
            >
          </a>
        </h6>
      </div>
