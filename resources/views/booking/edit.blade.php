                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-3">
                            ...
                            <a href="{{ route('booking.index') }}" class="btn btn-sm btn-primary float-right"><i
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            {{-- @Lee --}}
                            {{-- Price --}}
                            @if ($bookingdata->service->type == 'fixed' && $bookingdata->status == 'pending')
                                <div class="form-group col-md-4" id="fixed_price">
                                    ...
                                    <span class="pl-2"> {{ __('messages.price_ranger') }}
                                        ({{ $bookingdata->service->min_price_range }} - {{ $bookingdata->service->max_price_range }}) </span>
                                    {{ Form::number('price', $bookingdata->price ?? null, ['min' => $bookingdata->service->min_price_range, 'max' => $bookingdata->service->max_price_range, 'step' => '500', 'placeholder' => __('messages.price'), 'class' => 'form-control', 'required']) }}
                                </div>
                            @endif
                            {{-- End Price --}}
