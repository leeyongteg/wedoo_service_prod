...
class FrontendController extends Controller
{
    ...
    public function serviceDetail(Request $request)
    {
        ...
        if ($serviceData['service_detail']['type'] == 'hourly') {
            $price = $serviceData['service_detail']['price'];
            $discount = $serviceData['service_detail']['discount'];

            $subtotal = $discount != 0 ? $price - ($price * $discount / 100) : $price;
        } elseif ($serviceData['service_detail']['type'] == 'fixed') {
            $minRangePrice = $serviceData['service_detail']['min_price_range'];
            $maxRangePrice = $serviceData['service_detail']['max_price_range'];
            $discount = $serviceData['service_detail']['discount'];

            $subtotal_min = $discount != 0 ? $minRangePrice - ($minRangePrice * $discount / 100) : $minRangePrice;
            $subtotal_max = $discount != 0 ? $maxRangePrice - ($maxRangePrice * $discount / 100) : $maxRangePrice;
    ...
    public function bookServiceView(Request $request)
    {
        ...
        $service = Service::where('id', $service_id)->with('category', 'serviceRating', 'serviceAddon')->first();
