    ...
    public function index_data(DataTables $datatable)
    {
        ...
        return $datatable->collection($row)
            ...
            if (isset($row->data['check_booking_type'])) {
                return '<a class="btn-link btn-link-hover notify-table-link" href="' . route('booking.show', $row->data['id']) . '" >Booking # ' . $row->data['booking_id'] . ' ' . str_replace("_", " ", ucfirst($row->data['activity_type'])) . '</a>';
            } else {
                return '<a class="btn-link btn-link-hover notify-table-link" href="#" ># ' . $row->data['id'] . ' ' . str_replace("_", " ", ucfirst($row->data['activity_type'])) . '</a>';
            }
            })
            ...
            ->editColumn('action', function ($row) {
                if (isset($row->data['check_booking_type'])) {
                    return '<a href="' . route('booking.show', $row->data['booking_id']) . '"><span class="iq-bg-info mr-2"><i class="far fa-eye text-secondary"></i></span></a>';
                } else {
