...
class ServiceController extends Controller
{
  ...
  {
    // @Lee Retirer la relation providers dans la clause with
    $service = Service::where('service_type', 'service')->with(['category', 'serviceRating'])->orderBy('created_at', 'desc');
    ...
    if (
      $request->has('is_rating') && $request->is_rating != ''
    ) {
    ...
    if (
      $request->has('is_price_min') && $request->is_price_min != '' || $request->has('is_price_max') && $request->is_price_max != ''
    ) {
    ...
    if (
      $request->has('provider_id') && $request->provider_id != ''
    ) {
    ...
    if (
      $request->has('latitude') && !empty($request->latitude) && $request->has('longitude') && !empty($request->longitude)
    ) {
    ...
    $response = [
      ...
      'max' => ($service->max('max_price_range') > $service->max('price')) ? $service->max('max_price_range') : $service->max('price'),
      'min' => (($service->min('min_price_range') >  $service->min('price'))) ? $service->min('min_price_range') : $service->min('price'),
  ...
  public function getServiceDetail(Request $request)
  {
    ...
    if (auth()->user() !== null) {
      if (auth()->user()->hasRole('admin')) {
        // @Lee Retirer la relation providers dans la clause with
        $service = Service::where('service_type', 'service')->withTrashed()->with('category', 'serviceRating', 'serviceAddon')->findorfail($id);
      } else {
        // @Lee Retirer la relation providers dans la clause with
        $service = Service::where('service_type', 'service')->with('category', 'serviceRating', 'serviceAddon')->findorfail($id);
      }
    } else {
      // @Lee Retirer la relation providers dans la clause with
      $service = Service::where('service_type', 'service')->where('status', 1)->with('category', 'serviceRating', 'serviceAddon')->find($id);
    }
