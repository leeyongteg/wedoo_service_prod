...
trait NotificationTrait
{
    function sendNotification($data)
    {
        ...
        switch ($data['activity_type']) {
            case "add_booking":
                ...
                $this->notifyUser($admin_user, $data, $id);
                $this->notifyUser($booking->customer, $data, $id);
                break;

            case "assigned_booking_to_provider":
                $assigned_provider = isset($booking->provider) ? $booking->provider->display_name : '';

                $data['activity_message'] = __('messages.assigned_to_provider', ['name' => $assigned_provider]);
                $data['activity_type'] = __('messages.assigned_booking_to_provider');
                $provider_id = isset($booking->provider) ? $booking->provider_id : '';
                ...
                $this->notifyUser($booking->provider, $data, $id);
                $this->notifyUser($admin_user, $data, $id);
                break;
            ...
            case "assigned_booking":
                ...
                $handyman = User::where('id', $handymanId)->first();
                $activity_data = [
                ...
                if ($booking->provider->id != null)
                    $this->notifyUser($booking->provider, $data, $id);
                if ($handymanId != null)
                    $this->notifyUser($booking->handymanAdded[0]->handyman, $data, $id);
                break;

            case "transfer_booking_to_provider":
                $assigned_provider = isset($booking->provider) ? $booking->provider->display_name : '';

                $data['activity_type'] = __('messages.transfer_booking_to_provider');
                $data['activity_message'] = __('messages.booking_transfer_to_provider', ['name' => $assigned_provider]);
                $provider_id = isset($booking->provider) ? $booking->provider_id : '';
            ...
            case "payment_message_status":
                ...
                $this->notifyUser($admin_user, $data, $data['booking_id']);
                $this->notifyUser($booking->customer, $data, $data['booking_id']);
                if ($booking->provider->id != null)
                    $this->notifyUser($booking->provider, $data, $id);
                if ($handymanId != null)
                    $this->notifyUser($booking->handymanAdded[0]->handyman, $data, $id);
            ...
            case "define_price_of_service":
                $data['activity_type'] = __('messages.define_price_of_service_message');
                $data['activity_message'] = __(
                    'messages.define_price_of_service',
                    [
                        'id' =>  $data['booking_id'],
    ...
    private function notifyUser($user, $data, $id)
    {
        $message = $data['activity_message'];
        $activity_type = $data['activity_type'];
        $id = $id;
        $user->notify(new BookingNotification(array_merge($data, compact('message', 'id', 'activity_type'))));
    }
}
